...
Each territory contains it title, longtitude, latitude
Each sensor contains it's title, longtitude, latitude and value of the last probe (in microsieverts, μSv)
...
module.exports = function() {
    return request(config.LAST_MEASUREMENTS_URL).
        then(function(data) {
            // request passes two non-error arguments in the callback, 
            // so Bluebird's promisify method wraps them into array which passes to 'then' callback
            var response = data[0];
        ...
        then(function(data) {
            ...
            _.each(territories, function(territory) {
                ...
                // get longtitude and latitude from objects
                ...
                convertedTerritory["longtitude"] = Number.parseFloat(latLngContainer.lng);
