...
Sensor.prototype = {
     ...
     * @param {number|Date|*} [endDate=undefined] If you pass this argument, method will assume that you are interested in time frame for series of measurements
     ...
     * @param {{id: string, name: string, lng: string, lat: string, time: string, value: string}} rawData Fresh piece of JSONified XML data from server
     * @private
     */
    __parseAndSaveRawData: function(rawData) {
        if (rawData.id) {
            this.id = rawData.id.substr(~rawData.id.indexOf('_') ? rawData.id.indexOf('_') + 1 : 0);
        }
